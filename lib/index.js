...
export const Config = z.object({
  ...
  maxTextChars: z.number().min(0).max(200000).default(80000),
})

/**
 * Local settings helpers. Desktop 2.0.9's host resolve of `@deepseek-ai/dsh-settings`
 * does not expose `installSettingsSection` / `settingsNamespace` to linked plugins,
 * so the plugin keeps the same wiring without importing those named exports.
 */
function settingsNamespace(value) {
  if (typeof value !== 'string' || value.length === 0) {
    throw new Error('settings namespace must be a non-empty string')
  }
  return value
}
